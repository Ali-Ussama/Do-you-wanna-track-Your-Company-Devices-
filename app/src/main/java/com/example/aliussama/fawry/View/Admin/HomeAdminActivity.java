    ...
    private void exportMachinesIntoCSV(ArrayList<MachineModel> machines) {
        ...
        if (isExternalStorageWritable()) {
            ...
            try {
                ...
                bfWriter.write("Machine ID,Client Name,Client Phone,Address, Latitude, Longitude, Representative Name\n");

                for (MachineModel machine : machines) {
                    machine.setmAddress(machine.getmAddress().replaceAll(",", " - "));
                    machine.setmAddress(machine.getmAddress().replaceAll("\n"," -   "));
                    machine.setmClientName(machine.getmClientName().replaceAll("\n"," - "));
                    machine.setmClientPhone(machine.getmClientPhone().replaceAll("\n"," - "));
                    machine.setmRepresentativeName(machine.getmRepresentativeName().replaceAll("\n"," - "));


                    String seperator = ",";
                    bfWriter.write(machine.getmMachineId() + seperator + machine.getmClientName() + seperator + machine.getmClientPhone() + seperator + machine.getmAddress() + seperator + machine.getmLatitude() + seperator + machine.getmLongitude() + seperator + machine.getmRepresentativeName() + "\n");
                }
        ...
        try {
            ...
            if (usernameEditText.getText().toString().isEmpty()) {
                ...
                if (username.isEmpty()) {
                    ...
                    usernameEditText.setError("مطلوب");

                } else if (phoneNumber.isEmpty()) {
                    phoneEditText.setError("مطلوب");
                } else {
